...
- [x] Effects system fully implemented (steam, flame glow, water stream)
  - All effects disabled by default unless Workshop enables via `effects.json`
  - Flame image always shows; glow/animation are optional effects
  - Water stream is opt-in per Workshop
  - Workshop switching properly resets game (stage 0, force remount)
  - Documentation updated in GOTCHAS.md
- [x] Workshop system locked down and extensible
...
- [ ] Review data structure for custom Workshops:
  - `public/assets/workshops/<id>/workshop.json` – Document required vs. optional fields
  - Ensure new Workshops can override only specific colors (inheritance working?)
  - Test Workshop inheritance chain (alpha → alpha-alt → custom)
...
- [ ] Add file upload/validation for:
  - Custom fluid JSON
  - Custom workshop.json
  - Custom images (with size/format requirements)

## Priority 3: Documentation
- [ ] Update WORKSHOP_SYSTEM.md with:
- [ ] Create FLUID_CUSTOM_GUIDE.md for custom fluid creation
- [ ] Create EXTENSIBILITY_GUIDE.md for modders/Workshop creators
- [ ] Document all extensible data formats (fluids, Workshops, physics params)

## Next Session Context
- **Last work:** Effects system implementation (all effects opt-in per Workshop), Workshop switching reset, game extensibility
...
- **Workshop system:** ✅ Complete - validator fixed, effects opt-in, extensible, resets properly
